...
class group {
	constructor(p_source, n_workers=N_CORES, h_worker_options={}) {
		// omitted worker count argument; shift args
		if('number' !== typeof n_workers) {
			h_worker_options = n_workers;
			n_workers = N_CORES;
		}
		...
		// node inspect
		let h_inspect = h_worker_options.inspect;
		let i_inspect = h_inspect? h_inspect.range[0]: null;
		...
		for(let i_worker=0; i_worker<n_workers; i_worker++) {
			// fork options
			let h_options = Object.create(h_worker_options);

			// inspect port
			if(i_inspect && i_inspect <= h_inspect.range[1]) {
				let a_node_args = h_options.node_args = h_options.node_args? h_options.node_args.slice(0): [];
				a_node_args.push('--inspect'+(h_inspect.brk || h_inspect.break? '-brk': '')+'='+(i_inspect++));
			}
			...
			let k_worker = new worker({
				...
				options: Object.assign(h_options, {
					args: [String.fromCharCode(65+i_worker), ...(h_options.args || [])],
				}),
			});
